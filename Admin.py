...
def AdminHome(root, frame1, frame3):
    ...
    realTimeVoteCount = Button(frame1, text="Real-Time Vote Count", width=20, command=lambda: threading.Thread(target=adFunc.realTimeVoteCount, args=(root, frame1)).start())
    ...
    realTimeVoteCount.grid(row=9, column=1, columnspan=2)
